...
### T-test abd FDR correction of metabolites with VIPs >1
...
D37_BvsC_all_VIP_sig_accumulated <- D52_BvsC_all_VIP_sig2 %>%
  filter(Enrichment.dir != "Control")

D37_BvsC_all_VIP_sig_depleted <- D52_BvsC_all_VIP_sig2 %>%
  filter(Enrichment.dir == "Control")

D37_CvsP_all_VIP_sig_accumulated <- D52_CvsP_all_VIP_sig2 %>%
  filter(Enrichment.dir != "Control")

D37_CvsP_all_VIP_sig_depleted <- D52_CvsP_all_VIP_sig2 %>%
  filter(Enrichment.dir == "Control")
...
D37_accum <- D37_all_comp_accumulated %>%
  mutate(ordering = -as.numeric(Shared) + VIP,
         Metabolite = fct_reorder(Metabolite, ordering, .desc = T)) %>%
...
ggsave(filename="../../output/Metabolomics/Day37_accumulated_VIP.pdf", plot=D37_accum, dpi=300, width=7, height=9, units="in")

# 
# D37_accum <- ggplot(D37_all_comp_accumulated %>% arrage(Shared, desc(VIP)) %>%
#                        mutate(Metabolite=factor(Metabolite, levels = Shared)),
#                        aes(x = VIP, y = Metabolite, fill = Comparison)) +
#   geom_hline(aes(yintercept = Metabolite), linetype = "dotted", color = "grey") +
#   geom_point(size = 3, aes(color = Comparison)) +
# #  xlim(1.2, 2) +
#   scale_colour_manual(values=c("#8B0046", "#468B00")) +
