...
sample.info <- read.csv("../../data/Metabolomics/Porites-Kevin_SampleInfo2.csv")
...
#Determining number of compounds for each polarity/unknown

pos.comp <- unique(peak.pos4$compound)
length(pos.comp) # 77

neg.comp <- unique(peak.neg4$compound)
length(neg.comp) # 86

unknown.comp <- unique(peak.unknown4$compound)
length(unknown.comp) # 128
...
peak.all3 <- peak.all2 %>% dplyr::select(Sample.ID, compound, Fragment.ID, Day, Group, Norm.IonCount)
...
### Making a dataset for omic integration (Diablo)
norm.data.all$ColDay <- paste(norm.data.all$Fragment.ID, norm.data.all$Day, sep = "-")
rownames(norm.data.all) <- paste0(norm.data.all$ColDay)
metab.df <- norm.data.all %>% dplyr::select(-Sample.ID, -Fragment.ID, -Day, -ColDay)

write.csv(metab.df, "../../output/Metabolomics/metab_df.csv")
