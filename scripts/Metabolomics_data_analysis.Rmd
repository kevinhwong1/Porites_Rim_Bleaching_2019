...
1. Pre-data Processing
 - Import Data (Sample info, negative peaks, positive peaks)
 - Make a polarity column for each dataset and integrate into name
 - Bind data sets together
 - Normalize ion counts to sample input weight and make new dataframe
 - Select active columns
 - Zero inflate values by 1000
 - Log normalization
2. Preliminary PCA and statistics
...
## 1. Pre-data Processing: 
```{r Pre-data Processing, message=FALSE, warning=FALSE}
...
head(peak.all.active[ , c(1:2)])
```

## 2. Preliminary PCA and statistics

```{r Preliminary PCA and statistics, message=FALSE, warning=FALSE}
...
```{r Preliminary PLS-DA, message=FALSE, warning=FALSE}
...
```{r Validation of preliminary PLS-DA, message=FALSE, warning=FALSE}
...
```{r sPLS-DA from PLS-DA validation, message=FALSE, warning=FALSE}
...
#selectVar(MyResult.splsda.final, comp=4)$value
...
plotLoadings(MyResult.splsda.final, comp = 3, contrib = 'max', method = 'median')
plotLoadings(MyResult.splsda.final, comp = 4, contrib = 'max', method = 'median')

```

## 6. sPLS-DA model validation 

```{r sPLS-DA model validation, message=FALSE, warning=FALSE}
...
```{r Extraction of significant loadings, message=FALSE, warning=FALSE}
