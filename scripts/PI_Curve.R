...
#Project: Porites Nutrition 2019
#Author: HM Putnam 
#Edited by: KH Wong
#Date Last Modified: 20190605
...
##### PI Curve Rate Calculation 20190605 #####
path.p<-"data/PI_Curve_Data/20190605/" #the location of all your respirometry files 
...
Sample.Info <- read.csv(file="data/20190605_PI_Curve_Sample_Info.csv", header=T) #read sample.info data
...
#view rates
Photo.R
write.csv(Photo.R,"output/20190605_PI_Curve_rates.csv")
...
write.csv(Data,"output/20190605_PI_Curve_rates_Pastreoides.csv")

#Remove blanks
Data <- read.csv("output/20190605_PI_Curve_rates_Pastreoides.csv")
Data <- Data %>%
...
ggsave(file="output/PI_Curve_20190605.pdf", PI_curve, width = 11, height = 11, units = c("in"))
...
##### PI Curve Rate Calculation 2019707 #####
path.p<-"data/PI_Curve_Data/20190707/" #the location of all your respirometry files 
...
#Load Sample meta info Info
Sample.Info <- read.csv(file="data/20190707_PI_Curve_Sample_Info.csv", header=T) #read sample.info data
Sample.Info$Coral.ID <- paste(Sample.Info$Coral.ID,"_",Sample.Info$Light_Level,sep = "")
...
#view rates
Photo.R
write.csv(Photo.R,"output/20190707_PI_Curve_rates.csv")
...
write.csv(Data,"output/20190707_PI_Curve_rates_Pastreoides.csv")

#Remove blanks
Data <- read.csv("output/20190707_PI_Curve_rates_Pastreoides.csv")
Data <- Data %>%
  filter(Sample.Type != "Blank")
PI_curve <- ggplot(Data, aes(x = Light_Value, y = micromol.cm2.h, color = Sample.ID)) +
  geom_point() + 
  ...
  theme_bw() + theme(panel.border = element_rect(color="black", fill=NA, size=0.75), panel.grid.major = element_blank(), #Makes background theme white
                     panel.grid.minor = element_blank(), axis.line = element_blank()) 
ggsave(file="output/PI_Curve_20190707.pdf", PI_curve, width = 11, height = 11, units = c("in"))
...
##### PI Curve Rate Calculation 20190805 #####
path.p<-"data/July_Rim/PI_Curve_Data/20190805/" #the location of all your respirometry files 
...
#Load Sample meta info Info
Sample.Info <- read.csv(file="data/July_Rim/PI_Curve_Data/20190805_PI_Curve_Sample_Info.csv", header=T) #read sample.info data
Sample.Info$Coral.ID <- paste(Sample.Info$Coral.ID,"_",Sample.Info$Light_Level,sep = "")
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:length(lt.levs)){    
    ...
    #Save plot prior to and after data thinning to make sure thinning is not too extreme
    rename <- sub("_.*", "", file.names[i])
    pdf(paste0("output/PI_Curves/20190805",rename,"_",j,"thinning.pdf"))
    par(omi=rep(0.3, 4)) #set size of the outer margins in inches
    par(mfrow=c(1,2)) #set number of rows and columns in multi plot graphic
    plot(Value ~ sec, data=Photo.Data , xlab='Time (seconds)', ylab=substitute(' O'[2]~' (µmol/L)'),  axes=FALSE) #plot data as a function of time
    usr  <-  par('usr')
    ...
    Regs  <-  rankLocReg(xall=Photo.Data$sec, yall=Photo.Data$Value, alpha=0.2, 
                         method="pc", verbose=TRUE) 
    pdf(paste0("output/PI_Curves/20190805",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
...
#view rates
Photo.R
write.csv(Photo.R,"output/20190805_PI_Curve_rates.csv")
...
Data$Light_Level.Temp <- paste(Data$Light_Level,"_",Data$Temp.Cat,sep = "")

#calculate the average of the blanks from each time step
blnks <- subset(Data, Sample.Type=="Blank")
blnks <-mean.blnks <- aggregate(micromol.s ~ Light_Level.Temp, data=blnks, FUN=mean)
Data <- merge(Data, blnks, by="Light_Level.Temp")
colnames(Data)[colnames(Data) == 'micromol.s.x'] <- 'sample.micromol.s'
colnames(Data)[colnames(Data) == 'micromol.s.y'] <- 'blank.micromol.s'
...
write.csv(Data,"output/20190805_PI_Curve_rates_Pastreoides.csv")

#Remove blanks
Data <- read.csv("output/20190805_PI_Curve_rates_Pastreoides.csv")
Data <- Data %>%
  filter(Sample.Type != "Blank")
PI_curve <- ggplot(Data, aes(x = Light_Value, y = micromol.cm2.h, color = Sample.ID)) +
  geom_point() + 
...
#              size = 1, se = FALSE, colour = "blue") +
  theme_bw() + theme(panel.border = element_rect(color="black", fill=NA, size=0.75), panel.grid.major = element_blank(), #Makes background theme white
                     panel.grid.minor = element_blank(), axis.line = element_blank()) +
  facet_wrap(~Temp.Cat)
ggsave(file="output/PI_Curve_20190805.pdf", PI_curve, width = 11, height = 11, units = c("in"))
